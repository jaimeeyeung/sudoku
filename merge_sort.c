// merege(dest, src1, len1, src2, len2) modifies dest to contain the elements from both src1 and src2 in sorted order
...
// see merge.h for documentation
void merge_sort(int arr[], int arr_len) {
if (arr_len <= 1) return;
int llen = arr_len / 2;
int rlen = arr_len - llen;
